...
class InputObservable implements Observable {
  ...
  subscribe(...observers: Observer[]): void {
    observers.forEach((observer) => this.observers.includes(observer) || this.observers.push(observer));
  }
...
class ParagraphObserver implements Observer {
  ...
  update(observable: Observable): void {
    if (observable instanceof InputObservable) this.element.innerText = observable.element.value;
  }
}
